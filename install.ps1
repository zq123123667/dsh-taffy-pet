# 塔菲语音播报桌宠 —— 一键安装（Windows / PowerShell）
# 用法：在仓库根目录执行  .\install.ps1   （重复执行安全，幂等）
...
Write-Host "== [2/3] 挂载到 web profile：$ProfileRoot =="
...
"@ $PkgJson
    Write-Host '  已合并依赖到 package.json（保留原配置）'
} else {
    $pkgObj = @{
...
Write-Host ''
Write-Host '下一步：'
Write-Host '  1) 启动：  dsh web          （默认端口 3080）'
Write-Host '  2) 浏览器打开 http://127.0.0.1:3080'
Write-Host '  3) 右下角「🐱 启动塔菲桌宠」→ 展开即可使用'
Write-Host '  4) 想用高音质火山引擎：设置 → 插件 → 塔菲桌宠 填 Key（或环境变量 TAFFY_*）'
