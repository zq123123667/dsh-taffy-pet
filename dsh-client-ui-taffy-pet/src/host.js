// 塔菲语音播报桌宠 —— Host 半区（统一版 · 跨平台：Windows / macOS / Linux）
// 一份源码，两种安装模式共用：
...
//   - TTS 合成优先用 Node 内置 fetch（OpenSSL 栈）——修复 Windows 沙箱受限 token 下
//     curl/schannel 无法建立 HTTPS（SEC_E_NO_CREDENTIALS）的问题；受限动态沙箱无 fetch
//     时回退 shell + curl（macOS/Linux 正常）
//   - 不硬编码任何绝对路径；素材目录自动探测（TAFFY_ASSET_DIR 优先，其次 cwd 下的常见目录）
//   - 回退路径的 curl 命令仅用双引号 + 环境变量传参（sh / PowerShell 兼容，且杜绝注入）
//   - 不指定 workdir（使用执行器默认目录，任何平台都存在）
//
// 安全：fetch 路径密钥只进 HTTP 头（无 shell 注入面）；回退路径经 env 传入仅 "$VAR" 引用；
